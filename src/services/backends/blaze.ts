...
export class Blaze {
     ...
     */
    async send(cql: string, updateResponse: (response: ResponseStore) => void, controller?: AbortController, measureDefinitions: Measure[]): Promise<void> {
        try {
            updateResponse(new Map().set("DKTK", {
                    status: "claimed",
                    data: null,
                }));
